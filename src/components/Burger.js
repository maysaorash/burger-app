...
export default class Burger extends Component {
    ...
    burgerContent = () => {
        ...
        for (let i = 0; i < patty; i++){
            burger.push(<div key={burger.length} className="pattySide"></div>);
        }
        if(burger.length === 0)
        ...
        return (
            <>
                ...
                <div className="ingredientsBlock">
                    ...
                    <p>PATTY</p>
                    <div className="ingrBtns">
                        <button className="ingrBtn" onClick={() => this.addRemoveIngredient('add','patty')}>Add</button>
                        <button className="ingrBtn" onClick={() => this.addRemoveIngredient('remove','patty')}>Remove</button>
                    </div>
                </div>
            </>
        );
